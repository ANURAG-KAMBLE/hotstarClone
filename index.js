...
// Select all card containers, next buttons, and previous buttons
let cardContainers = [...document.querySelectorAll('.card-container')];
let preBtns = [...document.querySelectorAll('.pre-btn')];
let nxtBtns = [...document.querySelectorAll('.nxt-btn')];

// Loop through each card container and add click event listeners to the buttons
cardContainers.forEach((item, i) => {
  ...
  nxtBtns[i].addEventListener('click', () => {
    // Scroll the container to the right (increase scrollLeft) by a certain amount
    item.scrollLeft += containerWidth - 200; // You can adjust the scroll amount
  });

  // Add a click event listener to the previous button
  preBtns[i].addEventListener('click', () => {
    // Scroll the container to the left (decrease scrollLeft) by a certain amount
    item.scrollLeft -= containerWidth + 200; // You can adjust the scroll amount
  });
});
